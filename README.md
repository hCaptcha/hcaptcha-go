...
Set `HCAPTCHA_SECRET` and `HCAPTCHA_SITEKEY` in server configuration. Create the verifier at startup and reuse it across handlers. Exit before serving requests if either value is missing.
...
`Result` is `map[string]any` and preserves the Siteverify JSON fields. `VerifyRequest` returns an error for non-2xx HTTP status, transport failure, or invalid JSON. A valid response with `success: false` has no Go error. Deny the protected action in either case. Inspect `error-codes` for operational logging; do not expose raw verification details to users.
...
Use the same sitekey in the widget and `HCAPTCHA_SITEKEY`.
